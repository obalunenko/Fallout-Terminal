...
[[ -s "${resources_path}/sessions/demo-players.json" ]] || fail 'bundled demo player config is missing or empty'
cmp -s "${repository_root}/sessions/demo-players.json" "${resources_path}/sessions/demo-players.json" || fail 'bundled demo player config differs from the reviewed source resource'
[[ "$(stat -f '%Lp' "${resources_path}/sessions/demo-players.json")" == 444 ]] || fail 'bundled demo player config must be read-only mode 0444'
