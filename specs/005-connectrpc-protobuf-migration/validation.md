...
| Actual production ngrok boundary | `NGROK_TEST_URL=https://fallout-terminal.ngrok.app npx playwright test connectrpc-player.spec.mjs --grep 'actual authenticated ngrok endpoint'` | PASS; first snapshot, overlay dismissal, opaque recognition, and reconnect through the production Wails/ngrok process |
| Actual active-terminal ngrok boundary | `FIXTURE_PUBLIC_HOST=https://fallout-terminal.ngrok.app NGROK_TEST_URL=https://fallout-terminal.ngrok.app NGROK_TEST_FIXTURE=1 npx playwright test connectrpc-player.spec.mjs --grep 'actual authenticated ngrok endpoint'` | PASS; character selection, terminal render, master update, and reconnect through the real fixed domain |
