...
**Bugfix**: 2026-08-15 — [USER-SOAK-DECISION] Removed the 60-minute local and 30-minute authenticated-ngrok soak workloads from required migration acceptance; prior soak results remain historical, non-gating evidence.
...
- Q: Are the 60-minute local and 30-minute authenticated-ngrok workloads still required for migration acceptance? → A: No. Remove both long-duration workloads from the active acceptance gate and final-candidate sequence. Preserve already-recorded soak results only as historical evidence; package smoke, automated player/public-mode coverage, lifecycle cleanup, rollback, and conditional release-signing checks remain unchanged.
...
5. ~~**Given** external ngrok credentials or connectivity are unavailable, **When** final evidence is recorded, **Then** authenticated-ngrok soak results are `NOT RUN` and are not used as passing public-mode evidence.~~ Long-duration authenticated-ngrok soak evidence is no longer required for acceptance; public-mode security and fallback behavior remain covered by the existing automated and native checks.
...
- **SC-019**: ~~Before cutover, the required 60-minute local master/player soak satisfies its workload, median-RSS, convergence, listener, and cleanup thresholds, and the authenticated-ngrok soak either satisfies its complete 30-minute public workload or is explicitly `NOT RUN`, never a synthetic pass.~~ Both long-duration soak workloads are removed from acceptance; any existing results are historical and non-gating.
