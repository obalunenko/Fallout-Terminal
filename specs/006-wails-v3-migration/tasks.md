...
- [ ] T069 [US7] After T068, commit all prior evidence separately, commit the complete v2-free application/configuration/active-documentation source, require empty `git status --porcelain`, capture `BUILD_CANDIDATE_SHA=$(git rev-parse HEAD)` and the exact pin set, and only then record that identity in `docs/wails-v3-migration-rollback.md` and `specs/006-wails-v3-migration/quickstart.md`; before T070 and every later final task verify every tracked path except result/status fields in those two evidence files remains identical to the build candidate, never redefine it with an evidence-only commit, and restart the final sequence at T070 if application source, generated output, pins, or lockfiles change
...
- [ ] T079 Build the final personal-use macOS arm64 package from the unchanged build candidate, run `scripts/verify-macos-app.sh` and `scripts/wails-v3-cutover-check.sh`, repeat offline launch/one-listener/master-player-smoke/clean-quit against `build/bin/Fallout Terminal.app`, calculate its SHA-256, and record package results plus artifact identity in `specs/006-wails-v3-migration/quickstart.md`
- [ ] T080 Verify the unchanged T079 artifact SHA-256, execute the complete T067 60-minute local workload and RSS thresholds against `build/bin/Fallout Terminal.app`, verify the artifact digest again afterward, and record local-soak results against that exact identity in `specs/006-wails-v3-migration/quickstart.md`
- [ ] T077 Against the same unchanged build candidate and with real prerequisites, verify the T079 personal-use artifact SHA-256 before and after the complete T067 30-minute authenticated-ngrok workload, then run `scripts/build-macos.sh` Developer ID/notary/staple/DMG/Gatekeeper gates and record each separately produced release-artifact digest/profile; otherwise record each unavailable conditional result as `NOT RUN` in `specs/006-wails-v3-migration/quickstart.md`
- [ ] T078 Reconcile the immutable build candidate SHA, exact pins, commands, automated/manual results, `NOT RUN` reasons, artifact digests, final-candidate soak, rollback, and cutover status across `README.md`, `specs/006-wails-v3-migration/quickstart.md`, and `docs/wails-v3-migration-rollback.md`; limit the later evidence-only commit to result/status fields in the quickstart and Wails v3 rollback record, then rerun `scripts/wails-v3-cutover-check.sh` so active documentation is scanned after evidence updates, restarting T070 only if reconciliation changed application source, generated output, pins, or lockfiles

**Checkpoint**: Every required gate, including clean browser tests, final native journeys, root development launch, package smoke, and local soak, is tied to one clean committed build candidate SHA/pin set and one recorded artifact identity; conditional evidence is honest; no test or contract assertion was weakened to obtain acceptance. Any application source, generated-output, pin, or lockfile change during the final sequence invalidates later evidence and restarts Phase 11 at T070 against a new build candidate. Evidence-only result/status changes require the final T078 documentation/cutover rescan but do not redefine or rebuild the unchanged candidate.
...
- Final verification executes in file order as T070 → T071 → T072 → T073 → T074 → T075 → T076 → T079 → T080 → T077 → T078: tool/protobuf state → Wails bindings → locked frontend builds → Go tests → race tests → clean Playwright install/suite → final native journeys → package/offline smoke → local soak → conditional public soak/release → evidence reconciliation/final documentation scan. Only subcommands explicitly proven to use disjoint immutable inputs may run concurrently within one task.
