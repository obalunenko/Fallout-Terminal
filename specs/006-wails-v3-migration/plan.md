...
**Bugfix**: 2026-08-15 — [USER-SOAK-DECISION] Updated from bugfix patch; removed the mandatory 60-minute local and 30-minute authenticated-ngrok workloads from cutover and final-candidate acceptance.
...
**Testing**: Colocated table-driven Go tests with Testify, `t.Context()`, protobuf-aware comparison; direct JavaScript facade tests; existing Buf drift/breaking/negative/isolation gates; Playwright generated-player journeys; macOS bundle/signature inspection and manual smoke; prior soak results retained as historical evidence only
...
### Checkpoint 4 — CI, release automation, active docs, and rollback drill
...
- Sequentially verify tool/protobuf state, Wails bindings, both frontends/reproducible builds, Go tests, race tests, a clean locked Playwright installation and suite, root-development/native parity, final package/offline smoke, conditional release-signing gates, and reconciled documentation/evidence as separate attributable boundaries. The removed long-duration local and authenticated-ngrok soaks are not part of this sequence.
...
| Public release | release-script preflight and credential-backed sign/notary/staple/DMG/Gatekeeper only when available | installed package and trust checks | real evidence required; otherwise explicitly `NOT RUN`; no authenticated-ngrok duration workload |
| Rollback / conditional release | source/hash/data-safety assertions; v2/v3 forbidden scans | rollback drill using safety copies; conditional Developer ID/notary/staple/DMG/Gatekeeper checks; completed soak records retained as historical evidence only | rollback passes; release checks are real or explicitly `NOT RUN`; no long-duration soak acceptance gate |
...
- **Build candidate SHA**: `git rev-parse HEAD` captured only after the complete v2-free application/configuration source, `README.md`, and every active operating document are committed and `git status --porcelain` is empty. It identifies the immutable runtime inputs used for generation, tests, packaging, and native journeys. A later explicit user acceptance-scope decision may patch `spec.md`, `plan.md`, `tasks.md`, Companion context, quickstart results, and rollback results without redefining the runtime candidate, provided no application, build, generated, lock, active operating, or package input changes.
- **Personal-use bundle identity**: the canonical bundle-manifest SHA-256 produced by `scripts/hash-macos-app.sh`, whose byte-sorted manifest includes each bundle-relative entry's type, POSIX mode, regular-file content digest, or symlink target and excludes timestamps and host-specific extended attributes. Package verification and offline smoke verify that identity; historical soak evidence is non-gating.
